 ...
 * @provides xtrap
 * Project 5 - COSC 3250
 * Name: Vicenzo Infusino and Klemens Koszarek
 * Instructor: Dr. Brylow
 * TA-BOT:MAILTO vicenzo.infusino@marquette.edu klemens.koszarek@marquette.edu
 * This is the xtrap.c function implements system calls for Xinu. It's function
 * defaults to treating the SWI. 
 */
...
void xtrap(long *frame, int cause)
{
	...
	/* PSEUDOCODE:
	if ARM_EXCEPTION_SWI
		Find SWI (Software Interrupt) Address
		SWI_ADDRESS = FRAME[N}
		FRAME[N] += 4 //ARM ARCHITECTURE
		//RESET RETURN REGISTER IN FRAME
		FRAME[RET_REG] = SYSCALL_DISPACH(?,?)
		return;
	*/
	if (cause == ARM_EXCEPTION_SWI)
	{
		ulong swi;
		swi = *(ulong*)frame[CTX_LR]; //Finding the SWI Address
		frame[CTX_LR] = frame[CTX_LR] + 4; //incrementing link register to pass exception
		frame[CTX_R0] = syscall_dispatch(0xFFFFFF & swi, (int *) frame); //Resets return register in frame
		return;
	}
